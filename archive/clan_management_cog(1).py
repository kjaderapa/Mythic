...
class ClanManagement(commands.Cog):
    ...
    async def request_edit(self, ctx):
        """Request members to update their stats (officers only)"""
        # Check permissions
        if not any(role.name.lower() in ['officer', 'leader', 'admin'] for role in ctx.author.roles):
            embed = discord.Embed(
                title="❌ Permission Denied",
                description="Only officers can send edit requests!",
                color=Colors.ERROR
            )
            await ctx.send(embed=embed, ephemeral=True)
            return
        ...
        if not members:
            embed = discord.Embed(
                title="📭 No Members Found",
                description="No current members found to send requests to.",
                color=Colors.INFO
            )
            await ctx.send(embed=embed, ephemeral=True)
            return
        
        view = MemberSelectView(members, "request_edit")
        
        embed = discord.Embed(
            title="✏️ Request Stats Update",
            description="Select members to send edit requests to:",
            color=Colors.INFO
        )
        
        await ctx.send(embed=embed, view=view, ephemeral=True)
    
    @clan.command(name="leaderboard", description="View clan leaderboard")
    async def leaderboard(self, ctx):
        """View clan leaderboard with sorting options"""
        members = await self.bot.db.get_all_members(ctx.guild.id, "current")
        
        if not members:
            embed = discord.Embed(
                title="📭 No Members Found",
                description="No current members found for leaderboard.",
                color=Colors.INFO
            )
            await ctx.send(embed=embed)
            return
        
        # Filter out members without stats
        members_with_stats = [m for m in members if m.get('combat_rating') is not None]
        
        if not members_with_stats:
            embed = discord.Embed(
                title="📊 Leaderboard Empty",
                description="No members have entered their combat stats yet. Use `/profile` to add your stats!",
                color=Colors.INFO
            )
            await ctx.send(embed=embed)
            return
        
        view = LeaderboardView(members_with_stats)
        
        # Create initial leaderboard (sorted by combat rating)
        sorted_members = sorted(members_with_stats, key=lambda x: x.get('combat_rating', 0), reverse=True)
        
        embed = discord.Embed(
            title="🏆 Clan Leaderboard - Combat Rating",
            color=Colors.GOLD
        )
        ...
        for i, member in enumerate(sorted_members[:10], 1):
            ...
            combat_rating = member.get('combat_rating', 0)
            leaderboard_text += f"{rank_emoji} **{member['username']}** - {combat_rating:,}\n"
        
        embed.description = leaderboard_text
        embed.set_footer(text=f"Page 1 of {(len(sorted_members) - 1) // 10 + 1} • {len(sorted_members)} total members")
        
        await ctx.send(embed=embed, view=view)

class ManifestView(ui.View):
    def __init__(self, members: List[Dict]):
        super().__init__(timeout=300)
        self.members = members
    
    @ui.button(label="Leaderboard", style=discord.ButtonStyle.primary, emoji="🏆")
    async def show_leaderboard(self, interaction: discord.Interaction, button: ui.Button):
        # Filter members with stats
        members_with_stats = [m for m in self.members if m.get('combat_rating') is not None]
        
        if not members_with_stats:
            embed = discord.Embed(
                title="📊 No Stats Available",
                description="No members have entered their stats yet.",
                color=Colors.INFO
            )
            await interaction.response.send_message(embed=embed, ephemeral=True)
            return
